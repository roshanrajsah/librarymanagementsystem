...
async function loadBookForm() { await setOptions(); if (!bookId) return; const book = await api.get(`/books/${bookId}`); bookTitle.value = book.title; authorSelect.value = book.author_id; genreSelect.value = book.genre_id; stockInput.value = book.stock; descriptionInput.value = book.description || ''; document.getElementById('formTitle').textContent = 'Edit book'; document.getElementById('saveButton').textContent = 'Save changes'; }
