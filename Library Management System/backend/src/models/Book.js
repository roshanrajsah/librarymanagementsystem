...
const create = ({ title, authorId, genreId, stock, coverImage }) => new Promise((resolve, reject) => db.run('INSERT INTO books (title, author_id, genre_id, stock, cover_image) VALUES (?, ?, ?, ?, ?)', [title, authorId, genreId, stock, coverImage || null], function (error) { error ? reject(error) : resolve(findById(this.lastID)); }));
const update = (id, { title, authorId, genreId, stock, coverImage }) => new Promise((resolve, reject) => {
  const imageSql = coverImage ? ', cover_image = ?' : '';
  const values = [title, authorId, genreId, stock];
  if (coverImage) values.push(coverImage);
  values.push(id);
  db.run(`UPDATE books SET title = ?, author_id = ?, genre_id = ?, stock = ?${imageSql} WHERE id = ?`, values, function (error) { error ? reject(error) : this.changes ? resolve(findById(id)) : resolve(null); });
});
