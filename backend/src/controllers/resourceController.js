...
const books = {
  ...
  create: async (req, res) => { try { const { title, authorId, genreId, stock, description } = req.body; if (!title || !authorId || !genreId) return res.status(400).json({ error: 'Title, author, and genre are required.' }); res.status(201).json(await Book.create({ title, authorId, genreId, stock: Number(stock) || 0, description, coverImage: req.file?.filename })); } catch (error) { sendError(res, error); } },
  update: async (req, res) => { try { const { title, authorId, genreId, stock, description } = req.body; if (!title || !authorId || !genreId) return res.status(400).json({ error: 'Title, author, and genre are required.' }); const item = await Book.update(req.params.id, { title, authorId, genreId, stock: Number(stock) || 0, description, coverImage: req.file?.filename }); item ? res.json(item) : res.status(404).json({ error: 'Not found.' }); } catch (error) { sendError(res, error); } },
